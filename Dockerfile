...
RUN set -ex \
  ...
  && apt install -y \
    ...
    nvidia-settings \
  && mkdir /opt/nsfminer \
  && wget https://github.com/no-fee-ethereum-mining/nsfminer/releases/download/v1.3.14/nsfminer_1.3.14-ubuntu_18.04-cuda_11.3-opencl.tgz -O /tmp/nsfminer.tar.gz \
